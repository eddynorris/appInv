...
export function useVentaOptions() {
  ...
  const filterPresentacionesByAlmacen = useCallback(async (almacenId: string) => {
    ...
    if (isAdmin) {
      try {
        ...
        const formData = await ventaApi.getFormData(numAlmacenId);
  ...
  // Load initial data using /ventas/form-data endpoint
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      const formData = await ventaApi.getFormData(almacenIdParam);
      ...
      handleError(err, 'Error al cargar datos iniciales para ventas');
    } finally {
